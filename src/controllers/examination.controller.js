...
export const getExamById = async (req, res) => {
  try {
    ...
    if (!result.recordset.length) {
      return res
        .status(404)
        .json({ success: false, message: "Examination not found" });
    }
  ...
  try {
    ...
    if (!course_id || !exam_date || !exam_time || !hall) {
      return res
        ...
        .json({
          success: false,
          message: "course_id, exam_date, exam_time and hall are required",
        });
    }
...
export const upsertResult = async (req, res) => {
  try {
    ...
    if (!payload.length) {
      return res
        .status(400)
        .json({ success: false, message: "No result data provided" });
    }
    ...
    if (exam.recordset[0].Status !== "Completed") {
      return res
        ...
        .json({
          success: false,
          message: "Results can only be entered for Completed examinations",
        });
    }
    ...
    res
      .status(201)
      .json({ success: true, message: `${payload.length} result(s) saved` });
  } catch (err) {
